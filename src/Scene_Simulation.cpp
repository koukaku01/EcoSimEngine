...
Scene_Simulation::Scene_Simulation(SimulationEngine* simulation, std::string& levelPath)
    : Scene(simulation), m_levelPath(levelPath) {
    init(m_levelPath);
}

void Scene_Simulation::init(const std::string& levelPath) {
    loadLevel(levelPath);
...
void Scene_Simulation::loadLevel(const std::string& fileName) {
    ...
    if (!file) {
        std::cerr << "Scene_Simulation::loadLevel could not load " << fileName << " file.\n";
        exit(-1);
    }
    ...
    while (file >> entityType) {
        if (entityType == "Tile") {
            ...
            Vec2f boxSize = tile->get<CAnimation>().animation.getSize();
            boxSize -= Vec2f(2.0f, 2.0f);
            ...
            npc->add<CBoundingBox>(npc->get<CTransform>().pos, Vec2f(63.0f, 63.0f), false, false);
...
Vec2f Scene_Simulation::getPosition(int rx, int ry, int tx, int ty) const {
    // Implement this function, which takes in the room (rx, ry)
    // as well as the tile (tx, ty), and return the Vec2f game world
...
void Scene_Simulation::spawnPlayer() {
    ...
    player->add<CTransform>(Vec2f(m_playerConfig.X, m_playerConfig.Y));
    player->add<CAnimation>(m_game->assets().getAnimation("LinkStandDown"), true);
    player->add<CBoundingBox>(
        Vec2f(m_playerConfig.X, m_playerConfig.Y),
        Vec2f(m_playerConfig.CX, m_playerConfig.CY),
        true, false
    );
...
void Scene_Simulation::spawnSword(const std::shared_ptr<Entity>& entity) {
    ...
    Vec2f facing = entity->get<CTransform>().facing;
    Vec2f boxC = entity->get<CBoundingBox>().center;
    float swordL = 45.0f;
    float swordW = 15.0f;
    Vec2f swordCenter = Vec2f(
        boxC.x + facing.x * (swordL + 10.0f),
        boxC.y + facing.y * (swordL + 10.0f)
    );
    sword->add<CTransform>(swordCenter);
    if (facing.x != 0) {
        sword->add<CBoundingBox>(swordCenter, Vec2f(swordL, swordW), false, false);
    }
    else if (facing.y != 0) {
        sword->add<CBoundingBox>(swordCenter, Vec2f(swordW, swordL), false, false);
    }
...
void Scene_Simulation::sMovement() {
    ...
    for (const auto& entity : m_entityManager.getEntities()) {
        if (entity->has<CTransform>()) {
            ...
            //     Vec2f`& npcHome = entity->get<CFollowPlayer>().home;
            ...
            for (const auto& [name, anim] : m_game->assets().getAnimations()) {
                if (name.find("Tile") != std::string::npos) {
                    ...
                    if (ImGui::ImageButton(name.c_str(), anim.getSprite(), size)) {
                        ...
                        tile->add<CTransform>(Vec2f(view.x, view.y));
                        tile->add<CBoundingBox>(
                            Vec2f(view.x, view.y),
                            tile->get<CAnimation>().animation.getSize() - Vec2f(2.0f, 2.0f),
                            bm,
                            bv
                        );
                        tile->add<CDraggable>();
...
void Scene_Simulation::movementStart(const std::string& actionName) {
    if (!player()) return;
...
void Scene_Simulation::sDoAction(const Action& action) {
    ...
    if (action.type() == "START") {
        if (action.name() == "PAUSE") {
            ...
            Vec2f wPos = windowToWorld(m_mousePos);
            for (const auto& entity : m_entityManager.getEntities()) {
...
void Scene_Simulation::sAI() {
    ...
    for (const auto& npc : m_entityManager.getEntities("npc")) {
        ...
        Vec2f& pPos = p->get<CTransform>().pos;
        Vec2f& npcPos = npc->get<CTransform>().pos;
        ...
        if (npc->has<CPatrol>()) {
            ...
            Vec2f room = getRoomXY(npcTrans.pos);
            Vec2f toCoord = getPosition(
                room.x, room.y,
...
void Scene_Simulation::sCollision() {
    ...
    for (const auto& tile : m_entityManager.getEntities("tile")) {
        ...
        if (tile->get<CAnimation>().animation.getName() == "TileHeart") {
            for (const auto& entity : m_entityManager.getEntities()) {
                Vec2f overlap = Physics::GetOverlap(tile, entity);
    ...
    for (const auto& npc : m_entityManager.getEntities("npc")) {
        ...
        switch (ro.direction) {
        case ODirection::UP:
            if (npc->get<CTransform>().velocity != Vec2f(0, 0)) {
...
void Scene_Simulation::animatePlayer() {
    auto p = player();
    if (!p) return;
    ...
    if (p->get<CInput>().attack) {
        ...
        p->get<CTransform>().facing = Vec2f(1, 0);
        ...
        p->get<CTransform>().facing = Vec2f(0, -1);
        changePlayerStateTo("move", p->get<CTransform>().facing);
...
void Scene_Simulation::sAnimation() {
...
void Scene_Simulation::sRender() {
        ...
        for (const auto& entity : m_entityManager.getEntities()) {
            ...
            if (entity->has<CPatrol>()) {
                for (auto p : entity->get<CPatrol>().positions) {
                    Vec2f r = getRoomXY(entity->get<CTransform>().pos);
                    Vec2f pos = getPosition(r.x, r.y, p.x, p.y);
    ...
    if (m_drawGrid) {
        ...
        auto p = player();
        if (p) {
            Vec2f r = getRoomXY(p->get<CTransform>().pos);
...
std::shared_ptr<Entity> Scene_Simulation::player() {
...
void Scene_Simulation::changePlayerStateTo(const std::string& state, const Vec2f& facing) {
    auto p = player();
    if (!p) return;
    ...
    if (state == "stand") {
        ...
        else if (facing == Vec2f(-1, 0)) {
            newState = "stand_left";
        }
        else if (facing == Vec2f(0, -1)) {
            newState = "stand_up";
        }
        else if (facing == Vec2f(0, 1)) {
            newState = "stand_down";
    ...
    //    Vec2f offset = p->get<CBoundingBox>().center;
    if (state == "attack") {
        if (facing == Vec2f(1, 0)) {
...
Vec2f Scene_Simulation::windowToWorld(const Vec2f& pos) {
...
Vec2f Scene_Simulation::getRoomXY(const Vec2f& pos) {
